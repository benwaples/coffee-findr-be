...
app.get('/coffeeList', async(req, res) => {
  try {
    ...
    const data = await request
      .get(`https://api.yelp.com/v3/businesses/search?location=${location}&categories=coffee=sort_by=rating&limit=50`)
...
app.post('/favorites', async(req, res) => {
  try {
    ...
    const data = await client.query(`
        INSERT INTO favorites(biz_id, title, img, address, is_closed, rating, yelp_url, notes, owner_id)
        VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
        RETURNING * 
      `, [favoriteList.biz_id, favoriteList.title, favoriteList.img, favoriteList.address, favoriteList.is_closed, favoriteList.rating, favoriteList.yelp_url, favoriteList.notes, userId]);
